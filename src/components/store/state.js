export default {
  userData: [

  ],
  temp: '',
  data: [],
  todos: [
    { "completed": false, "id": 201, "title": "Standup meeting with the team @5pm", "description": null, "inDate": "Jul 10", "priority": "Low", "tags": [{ "name": "Personal", "color": "rgb(102, 204, 255)" }], "priorityColor": "#5e72e4" },
    { "completed": false, "id": 202, "title": "Order pizza for Granny tonight", "description": "Thin crust medium size roasted chicken", "inDate": "Jul 10", "priority": "High", "tags": [{ "name": "Home", "color": "rgb(255, 203, 0)" }, { "name": "Personal", "color": "rgb(102, 204, 255)" }], "priorityColor": "#f5365c" },
    { "completed": false, "id": 203, "title": "Design, Develop and Deploy Apps to Netlify for Clients", "description": null, "inDate": "Jul 10", "priority": "Medium", "tags": [{ "name": "Home", "color": "rgb(255, 203, 0)" }], "priorityColor": "#5e72e4" }
  ],
  allTags: [
    {
      name: "Home",
      color: "rgb(255, 203, 0)"
    },
    {
      name: "Personal",
      color: "rgb(102, 204, 255)"
    },
    {
      name: "Work",
      color: "rgb(162, 93, 220)"
    }
  ],
  defaultTagColor: "rgb(128, 128, 128)",
  colorPalete: [
    "#00C821",
    "#9CD326",
    "#CAB641",
    "#FFCB00",
    "#784BD1",
    "#A25DDC",
    "#0086C0",
    "#579BFC",
    "#66CCFF",
    "#BB3354",
    "#E2445C",
    "#FF158A",
    "#FF5AC4",
    "#FF642E",
    "#FDAB3D",
    "#7F5347",
    "#C4C4C4",
    "#808080",
    "#333333"
  ],
}